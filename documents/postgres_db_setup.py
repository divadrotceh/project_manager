...
DECLARE
    v_user_id BIGINT;
BEGIN
    INSERT INTO app_user (u_ser_name, u_pwd)
    VALUES (p_u_ser_name, crypt(p_plain_pwd, gen_salt('bf')))
...
BEGIN
    ...
    RETURN QUERY
    SELECT p.p_id, p.p_name, p.p_description, p.p_owner_u_id, p.p_created_at, p.p_updated_at
      FROM project p
     WHERE p.p_id = p_p_id;
END;
$$;
